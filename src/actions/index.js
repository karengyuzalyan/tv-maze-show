...
export function getShowByID(ID) {
    return dispatch => {
        dispatch({ type: GET_SHOW_BY_ID_STARTED, pending: 0 });

        axios.get(`/shows/${ID}`)
        .then(res => {
            dispatch({ type: GET_SHOW_BY_ID_SUCCESS, data: res, pending: 1 });
        })
        .catch(err => {
...
export function getEpisodeByNumber(query) {
    return dispatch => {
        dispatch({ type: GET_EPISODE_BY_NUMBER_STARTED, pending: 0 });

        axios.get(`${query}`)
        .then(res => {
            dispatch({ type: GET_EPISODE_BY_NUMBER_SUCCESS, data: res, pending: 1 });
        })
        .catch(err => {
...
export function getEpisodesByShow(ID) {
    return dispatch => {
        dispatch({ type: GET_EPISODES_BY_SHOW_STARTED, pending: 0 });

        axios.get(`/shows/${ID}/episodes`)
        .then(res => {
            dispatch({ type: GET_EPISODES_BY_SHOW_SUCCESS, data: res, pending: 1 });
        })
        .catch(err => {
